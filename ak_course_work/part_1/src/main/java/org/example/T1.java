...
public class T1 extends Thread{
    ...
    public T1(int start, int end, Resources resources, Semaphore sem, CyclicBarrier enteringBarrier, CyclicBarrier mainBarrier,
              CyclicBarrier endingBarrier, CyclicBarrier prepareToMinOperationBarrier) {
        this.start = start;
        this.end = end;
...
//        }
        ...
        for (int i = start; i < end; i++)
            partOfc += resources.B[i] * resources.Z[i];
        try{
            sem.acquire();
            resources.c += partOfc;
        }catch(InterruptedException exc){
